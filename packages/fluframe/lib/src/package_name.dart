...
/// `order_history` → `OrderHistory`, for a class name.
String pascalCase(String name) => humanizePackageName(name).replaceAll(' ', '');

/// `order_history` → `orderHistory`, for an identifier.
///
/// Feature names are snake_case (`packageNameRejection` accepts
/// `^[a-z][a-z0-9_]*$`), and Dart's `non_constant_identifier_names` — on
/// in `very_good_analysis`, which every generated app includes — rejects
/// a snake_case variable. Emitting `${name}ControllerProvider` verbatim
/// therefore produced `order_historyControllerProvider` and made the app
/// fail its own `flutter analyze` (#162).
String lowerCamelCase(String name) {
