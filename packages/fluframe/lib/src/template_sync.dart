/// Building `templates/` — the copy of the monorepo template that ships
/// inside the published package.
///
/// The work lives here rather than in `tool/sync_template.dart` because
/// every branch of it is a refusal: a missing template, a missing
/// `.gitignore`, an entry that vanished mid-run, a secret-like file in the
/// finished bundle. `tool/publish.bat` and the publish workflow both gate
/// on the exit code, and none of those branches had a test — the tool was
/// only ever run end to end, against the real checkout, where none of them
/// fire.
library;
...
/// Copies the overlay subset of [repoRoot]'s `template/` (and
/// `template_addons/`) into `[packageRoot]/templates`, returning the exit
/// code the tool should carry: 0 when the bundle is complete and clean, 1
/// otherwise.
///
/// [out] and [err] default to stdout and stderr. Every sentence written to
/// them is part of the tool's contract — `tool/publish.bat` and
/// `.github/workflows/publish.yml` read the exit code, and
/// `create_e2e_test.dart` runs the tool itself — so wording changes here
/// are changes to that contract.
///
/// [onSourcesVerified] runs between the completeness gate and the copy
/// loop, and is null in every real run. It exists so a test can construct
/// the one branch that no filesystem state can otherwise produce: the gate
/// and the copy loop decide "is this entry there?" with the same two
/// predicates, so an entry can only be absent at the second one by
...
int syncTemplate({
  ...
  final repoTemplate = Directory(p.join(repoRoot.path, 'template'));
  if (!repoTemplate.existsSync()) {
    errors.writeln('Repo template not found at ${repoTemplate.path}.');
    return 1;
  }
  ...
  if (!templateGitignore.existsSync()) {
    ...
    errors.writeln('template/.gitignore not found — refusing to sync.');
    return 1;
  }

  // Backend addon sources (ADR 0001) ship alongside the base template.
  final repoAddons = Directory(p.join(repoRoot.path, 'template_addons'));
  ...
  // Assigned, never lowered: an entry that vanishes mid-sync fails the run
  // even though everything after it succeeds, and a clean secret scan at
  ...
  for (final entry in overlayEntries) {
    ...
    if (FileSystemEntity.isDirectorySync(source)) {
      ...
      errors.writeln('Template entry "$entry" vanished during the sync.');
      failed = true;
    }
  }
