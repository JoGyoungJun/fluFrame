...
- Every UI string goes into ALL THREE ARBs — `lib/l10n/app_en.arb`,
  `app_ja.arb` and `app_ko.arb` — then `flutter gen-l10n`. Never hardcode
  UI text. Use ICU plurals for counts. (`gen-l10n` falls back to English
  and only warns for a missing key, so a skipped locale ships silently;
  `test/l10n/arb_parity_test.dart` is what catches it.)
...
- Keep all four rename tokens intact: `fluframe_app`, `FluFrame App`,
  `FluFrame 앱`, `FluFrame アプリ` (see `project_generator.dart:14-25`).
  Use package imports (`package:fluframe_app/...`).
