...
Two cautions, both established in CLAUDE.md's "Project goal":

- **Do not track dependent repos or dependent packages.** They are
  structurally 0 — a generated app carries no `fluframe` manifest entry —
  so reporting them as a metric reads as a trend when it is an artifact.
- **Downloads are not users.** pub.dev counts archive fetches and warns
  they are inflated by CI systems. Report the number, but do not
  characterise a change in it as adoption without a referrer or star
  movement to corroborate it.

When the honest finding is "no engineering in this milestone could have
moved these numbers", write that instead of an action item that implies
otherwise.
