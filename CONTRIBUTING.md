...
5. If your change moves or renames something a document points at — a
   ...
   the **same PR**. This applies to `.claude/skills/*/SKILL.md`,
   `CLAUDE.md`, `README.md` and `docs/`. Those files are read
   occasionally and rot silently between readings: `/release` spent four
   versions telling a releaser to edit `cliVersion` in a file that no
   longer declared it.
