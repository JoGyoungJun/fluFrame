...
> Historical record: current behaviour is defined by the code and CHANGELOG.

**Status**: APPROVED · **Size**: Feature
...
`packages/fluframe/test/feature_scaffold_test.dart` (unit, offline,
against a fixture app):
...
Satisfied by the test plan above: the five files are created with the
app's own package name, `--tab` registers branch + destination, all three
ARBs receive the keys with untranslated ones named, every refusal exits
non-zero before any write, and `flutter analyze` + `flutter test` pass in
the generated app.
