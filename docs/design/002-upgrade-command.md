...
> Historical record: written against fluframe v1.0.0; current behaviour is defined by the code and CHANGELOG.

- **Status**: APPROVED
- **Shipped in**: v1.0.0 (generation metadata + upgrade command)
- **Date**: 2026-08-03
...
**Metadata (prerequisite)** — `create` writes `.fluframe.json` to
...
**Upgrade** — `fluframe upgrade [--apply] [--from <ver>]
[--project-dir <dir>]`:
...
> **Superseded (1.2.0):** the shipped implementation invokes `git merge-file` **without** `-p`, so merged bytes are read back from the OURS file instead of stdout — see ADR 0002, *Amendment: apply safety*.
...
- [ ] `--apply` writes clean merges + conflict markers, bumps metadata,
  ...
  > **Amended (1.2.0):** `.fluframe.json` advances only on a fully clean result; a conflicted run leaves the recorded version alone and exits non-zero. See ADR 0002.

- [ ] git absent → graceful degradation to report-only with a message
- [ ] Unit suite offline; e2e exercises a real historical version
- [ ] Release tooling + CONTRIBUTING updated for `version.dart`
