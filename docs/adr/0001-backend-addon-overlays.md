...
> Historical record: written against fluframe v0.4.0 and amended at 1.2.0; current behaviour is defined by the code and CHANGELOG.

- **Status**: Accepted (amended 2026-08-06 — see *Amendment: what the
  addon mechanism owes downstream*)
- **Date**: 2026-08-03
- **Related**: design spec 001 (auth scaffold); shipped in v0.4.0
...
pin `authRepositoryProvider` to the in-memory implementation, so
...
the scheduled dependency-watch and dependency-upgrade passes.
