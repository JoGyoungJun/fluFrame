...
- **Status**: Accepted
- **Date**: 2026-08-06
- **Related**: `template/lib/features/*/presentation/`, CLAUDE.md's
  post-cutoff Riverpod notes; recorded retroactively while auditing 1.1.0
...
provider, paired with
`final postsControllerProvider = AsyncNotifierProvider<PostsController,
List<Post>>(PostsController.new);`.
...
- Adding a provider needs no `build_runner` run. `/new-feature` and the
  guides can say "write this file"; the CI codegen check stays about
  models and ARB files only.
